...
class HubScanCallback : public NimBLEAdvertisedDeviceCallbacks {
  void onResult(NimBLEAdvertisedDevice* device) {
    ...
    // CBM (capteur_bassins): bassin 2 et 3
...
void bleScan() {
  ...
  Serial.println("BLE: scan 10s...");
  NimBLEScan* pScan = NimBLEDevice::getScan();
  pScan->start(10, false);
  Serial.printf("BLE: terminé, %d appareils\n", pScan->getResults().getCount());
  pScan->clearResults();
}
...
void loop() {
  ...
  // Scan BLE toutes les 20s (CBM bassins + Inkbird temp/hum)
  ...
  // Dompeur: publier elapsed (secondes) et live (MM:SS) toutes les secondes
  if (mqtt.connected() && millis() - lastDompeurMqtt >= 1000) {
    ...
    if (!dompeurReset && lsLastEdge > 0) {
      ...
      // MM:SS pour compatibilité avec l'ancien topic dompeur/live
      char liveBuf[8];
      snprintf(liveBuf, sizeof(liveBuf), "%02lu:%02lu", sec / 60, sec % 60);
      mqtt.publish(mqttTopicDompeurLive.c_str(), liveBuf, true);
      // Secondes entières — source de vérité pour l'affichage du timer sur le dashboard
      char elapsedBuf[12];
